...
while {_equal && _counter < 500} do 
{
	_pos = [cpg_v_cpos, cpg_side_adder, (1000*cpg_side_mult), 4, 0, 0, 0, cpg_v_blacklist, cpg_v_dpos] call BIS_fnc_findSafePos;
	_check = _pos isflatempty [ 10, 0, 0.3, 25, 0, false, objNull]; 
	_counter = _counter + 1;
	_equal = [[], _check] call BIS_fnc_areEqual;		
};
...
{
	_pos = [cpg_v_cpos, 0, (1600*cpg_mult), 4, 0, 0, 0, cpg_v_blacklist, cpg_v_dpos] call BIS_fnc_findSafePos;
};
...
{
	nul = [_pos2, 2, _grp, 0] call cpg_fnc_spawnVehicles;
};
...
campaign_balance = campaign_balance + 0.075;
