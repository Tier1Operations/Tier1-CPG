// Spawn the Jet if player is a pilot

_caller = _this select 1;
	
if (_caller isKindOf "B_Pilot_F") then {
	...
	if (local (_this select 1)) then {
		if (sdelay > 60) then {
			...
			_pos = [(getPos spawnJet1), 0, 30, 13, 0, 1000, 0, [], [(getPos spawnJet1),(getPos spawnJet1)]] call BIS_fnc_findSafePos;
			_veh = "I_Plane_Fighter_03_CAS_F" createVehicle _pos;
			_veh setDir (getDir spawnJet1);
...
// Send vehicle to server, so that the server can protect the pilot seat.
	...
	hintSilent "You cannot spawn a Jet if you are not a pilot!";
	playSound "warning1";
